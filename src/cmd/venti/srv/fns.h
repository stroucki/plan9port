...
void		addstat(uint, long);
void		addstat2(uint, long, uint, long);
ZBlock		*alloczblock(u64int size, int zeroed, u64int alignment);
...
void		dirtydblock(DBlock*, u32int);
void		diskaccess(int);
void		disksched(void);
void		*emalloc(ulong);
...
void		initdcache(ulong mem);
void		initicache(ulong mem);
...
void		initlumpcache(ulong size, u32int nblocks);
...
int		maparenas(AMap *am, Arena **arenas, u32int n, char *what);
...
u32int		now(void);
int		okamap(AMap *am, u32int n, u64int start, u64int stop, char *what);
...
long		readarena(Arena *arena, u64int aa, u8int *buf, u64int n);
int		readarenamap(AMapN *amn, Part *part, u64int base, u32int size);
Bloom	*readbloom(Part*);
int		readclumpinfo(Arena *arena, u32int clump, ClumpInfo *ci);
int		readclumpinfos(Arena *arena, u32int clump, ClumpInfo *cis, int n);
ZBlock		*readfile(char *name);
int		readifile(IFile *f, char *name);
Packet		*readlump(u8int *score, int type, u32int size, int *cached);
vlong		readpart(Part *part, u64int addr, u8int *buf, u64int n);
...
int		writeclumpinfo(Arena *arean, u32int clump, ClumpInfo *ci);
...
vlong		writepart(Part *part, u64int addr, u8int *buf, u64int n);
